...
class IdentityProviderBaseTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.oidc_client = OIDCConfidentialClientFactory.create(
            response_types=[oidc_models.ResponseType.objects.get(value="code")]
        )
    ...
    def authkey_introspect(self, web_client: Client, session_token: str):
        ...
        """
        return web_client.get(
            reverse("authkey_introspect"), {"authkey": session_token,},
        )
...
class TestOpenIdCustomization(IdentityProviderBaseTestCase):
    def test_stateless_authorize_response(self):
        ...
        # OIDC authorize
        authorize_response = self.oidc_authorize(
            web_client, view_name="stateless_authorize"
        )

        self.assertEqual(
            authorize_response.status_code,
            302,
            "OIDC /authorize didn't return 302 redirection response.",
        )
        self.assertIn(
            "code=",
            authorize_response["Location"],
            "OIDC /authorize didn't return a code",
        )
        self.assertNotIn(
            "state=",
            authorize_response["Location"],
            "Geoserver OIDC /authorize customization returned empty state parameter",
        )
    ...
    def test_authorize_djam_session_update(self):
        ...
        oidc_code = re.search(
            r"code=(\w+)&?.*$", authorize_response["Location"]
        ).groups()[0]
        djam_session = models.Session.objects.filter(oidc_code=oidc_code).first()

        self.assertIsNotNone(
            djam_session,
            "OIDC /authorize didn't update the user session with oidc code",
        )
    ...
    def test_token_with_session_key(self):
        ...
        self.assertEqual(
            ...
            200,
            "OIDC /token: response status code is not 200",
        )
        self.assertIn(
            "access_token",
            token_response.json(),
            "OIDC /token: access_token not in JSON response",
        )
        self.assertIn(
            "id_token",
            token_response.json(),
            "OIDC /token: id_token not in JSON response",
        )
        self.assertIn(
            "session_token",
            token_response.json(),
            "OIDC /token: session_token not in JSON response",
        )
        self.assertIsNotNone(
            token_response.json().get("session_token"),
            "OIDC /token: session_token in JSON response is None",
        )
        self.assertIn(
            "expires_in",
            token_response.json(),
            "OIDC /token: expires_in not in JSON response",
        )
    ...
    def test_require_https_for_geoserver_token_introspection(self):
        ...
        self.access_token_intorspect(
            web_client, token_response.json().get("access_token")
        )

        # Introspect access token
        with self.settings(REQUIRE_SECURE_HTTP_FOR_GEOSERVER_INTROSPECTION=True):
            access_token_response = self.access_token_intorspect(
                web_client, token_response.json().get("access_token")
            )
        ...
        self.assertEqual(
            ...
            400,
            "OIDC token introspection with HTTPS required: response status code is not 400",
        )
...
class TestAuthKey(IdentityProviderBaseTestCase):
    ...
    def test_validate_invalid_session_key(self):
        ...
        self.assertEqual(
            authkey_response.status_code,
            404,
            "/authkey/introspect with invalid session_token: response status code is not 404",
        )
        self.assertIn(
            "username",
            authkey_response.json(),
            "/authkey/introspect with invalid session_token: username not in JSON response",
        )
        self.assertIn(
            "groups",
            authkey_response.json(),
            "/authkey/introspect with invalid session_token: groups not in JSON response",
        )
        self.assertIsNone(
            authkey_response.json().get("username"),
            "/authkey/introspect with invalid session_token: username is not None",
        )
        self.assertIsNone(
            authkey_response.json().get("groups"),
    ...
    def test_validate_valid_api_key(self):
        ...
        self.assertEqual(
            apikey_response.status_code,
            200,
            "/authkey/introspect API key: response status code is not 200",
        )
        self.assertIn(
            "username",
            apikey_response.json(),
            "/authkey/introspect API key: username not in JSON response",
        )
        self.assertIn(
            "groups",
            apikey_response.json(),
            "/authkey/introspect API key: groups not in JSON response",
        )
        self.assertEqual(
            apikey_response.json().get("username"),
            user.email,
    ...
    def test_validate_revoked_api_key(self):
        ...
        self.assertEqual(
            apikey_response.status_code,
            404,
            "/authkey/introspect revoked API key: response status code is not 200",
        )
        self.assertIn(
            "username",
            apikey_response.json(),
            "/authkey/introspect revoked API key: username not in JSON response",
        )
        self.assertIn(
            "groups",
            apikey_response.json(),
            "/authkey/introspect revoked API key: groups not in JSON response",
        )
        self.assertIsNone(
            apikey_response.json().get("username"),
            "/authkey/introspect revoked API key: username in JSON response is not None",
        )
        self.assertIsNone(
            apikey_response.json().get("groups"),
            "/authkey/introspect revoked API key: groups in JSON response is not None",
        )
    ...
    def test_validate_valid_api_key_with_existing_session_key(self):
        ...
        self.assertEqual(
            apikey_response.status_code,
            200,
            "/authkey/introspect API key with existing SessionKey: response status code is not 200",
        )
        self.assertIn(
            "username",
            apikey_response.json(),
            "/authkey/introspect API key with existing SessionKey: username not in JSON response",
        )
        self.assertIn(
            "groups",
            apikey_response.json(),
            "/authkey/introspect API key with existing SessionKey: groups not in JSON response",
        )
        self.assertEqual(
            apikey_response.json().get("username"),
            user.email,
    ...
    def test_validate_valid_session_key_with_existing_api_key(self):
        ...
        self.assertEqual(
            authkey_response.status_code,
            200,
            "/authkey/introspect Session key with existing API Key: response status code is not 200",
        )
        self.assertIn(
            "username",
            authkey_response.json(),
            "/authkey/introspect Session key with existing API Key: username not in JSON response",
        )
        self.assertIn(
            "groups",
            authkey_response.json(),
            "/authkey/introspect Session key with existing API Key: groups not in JSON response",
        )
        self.assertEqual(
            authkey_response.json().get("username"),
            user.email,
    ...
    def test_require_https_for_geoserver_authkey_introspection(self):
        ...
        self.assertEqual(
            authkey_response.status_code,
            400,
            "/authkey/introspect with HTTPS required: response status code is not 400",
        )
        self.assertIn(
            "username",
            authkey_response.json(),
        ...
        self.assertIsNone(
            authkey_response.json().get("username"),
            "/authkey/introspect: returned username is not None",
        )
        self.assertIsNone(
            authkey_response.json().get("groups"),
            "/authkey/introspect: returned groups is not None",
        )
...
class TestUserCredentialsValidation(TestCase):
    def validate_user_credentials(self, web_client, username, password):
        ...
        """
        return web_client.get(
            reverse("user_credentials_introspection"), {"u": username, "p": password,},
        )
    ...
    def test_user_credentials_validation_with_correct_credentials(self):
        ...
        self.assertIn(
            "username",
            user_credentials_response.json(),
            "User credentials response: username not in JSON response",
        )
        self.assertEqual(
            user_credentials_response.json().get("username"),
            user.username,
            "User credentials response: username is wrong",
        )
        self.assertIn(
            "groups",
            user_credentials_response.json(),
            "User credentials response: groups not in JSON response",
        )
        self.assertIsNotNone(
            user_credentials_response.json().get("username"),
            "User credentials response: returned username is None",
        )
        self.assertIsNotNone(
            user_credentials_response.json().get("groups"),
            "User credentials response: returned groups is None",
        )
    ...
    def test_user_credentials_validation_with_wrong_credentials(self):
        ...
        self.assertIsNone(
            user_credentials_response.json().get("username"),
            "User credentials response: returned username is not None",
        )
        self.assertIsNone(
            user_credentials_response.json().get("groups"),
            "User credentials response: returned groups is not None",
        )
    ...
    def test_require_https_for_user_credentials_validation(self):
        ...
        self.assertEqual(
            user_credentials_response.status_code,
            400,
            "User credentials response: response status code is not 400",
        )
        self.assertIn(
            "username",
            user_credentials_response.json(),
